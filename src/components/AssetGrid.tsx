        ...
        return (
          <div key={asset.id} className="card group cursor-pointer flex flex-col">
            <div className="relative aspect-video overflow-hidden" style={{ backgroundColor: 'var(--bg-subtle)' }}>
              {asset.thumbnail ? (
                <img src={asset.thumbnail} alt={asset.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
              ) : (
                <div className="w-full h-full flex items-center justify-center text-3xl">{typeStyle.icon}</div>
              )}

              <span className="absolute top-2 left-2 badge text-xs font-semibold"
                style={{ backgroundColor: typeStyle.bg, color: typeStyle.color, backdropFilter: 'blur(6px)' }}>
                {typeStyle.icon} {asset.type}
              </span>

              <span className="absolute top-2 right-2 badge text-xs font-medium"
              ...
              {asset.tags.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-auto">
                  {asset.tags.slice(0, 3).map(tag => (
                    <span key={tag} className="text-xs rounded px-2 py-0.5"
